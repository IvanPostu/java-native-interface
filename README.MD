...
- Java requires `field.setAccessible(true)` in order to access a private field
  ...
  bash "$PROJECT_ROOT/projects/test1cpp/scripts/build-so.sh" "$PROJECT_ROOT/projects/test1cpp/src/jni/test1java_TestNative" "$PROJECT_ROOT/projects/test1java/src/jni"
